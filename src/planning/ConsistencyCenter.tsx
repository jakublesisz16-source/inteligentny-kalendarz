...
export function ConsistencyCenter({ issues, events, onEdit, onAcknowledge, onStudySeriesCorrect }: ConsistencyCenterProps) {
  ...
  return <section className="panel consistency-center" id="consistency-center">
    ...
    <div className="consistency-toolbar">
      <div className="consistency-filters consistency-filters-desktop">{(['ALL','STUDY','WORK','PERSONAL','OVERLAP','TOUCHING'] as Filter[]).map((item) => <button type="button" key={item} className={filter === item ? 'choice-button active' : 'choice-button'} onClick={() => setFilter(item)}>{({ALL:'Wszystkie',STUDY:'Zajęcia',WORK:'Praca',PERSONAL:'Prywatne',OVERLAP:'Nakładanie',TOUCHING:'Brak buforu'} as Record<Filter,string>)[item]}</button>)}</div>
      <label className="consistency-filter-select"><span>Filtr</span><select value={filter} onChange={(event) => setFilter(event.target.value as Filter)}>{(['ALL','STUDY','WORK','PERSONAL','OVERLAP','TOUCHING'] as Filter[]).map((item) => <option key={item} value={item}>{({ALL:'Wszystkie',STUDY:'Zajęcia',WORK:'Praca',PERSONAL:'Prywatne',OVERLAP:'Nakładanie',TOUCHING:'Brak buforu'} as Record<Filter,string>)[item]}</option>)}</select></label>
    ...
    {visible.length ? <div className="consistency-list">{visible.map((issue) => {
      ...
      return <article className={`consistency-card impact-${issue.planningImpact.toLowerCase()}${issue.acknowledged ? ' acknowledged' : ''}`} key={issue.fingerprint}>
        ...
        {related.length ? <div className="consistency-events">{related.map((event) => <div key={event.id}><span className={`category-dot category-${event.category.toLowerCase()}`} aria-hidden="true"/><div><strong>{event.title}</strong><small>{event.startDateTime.slice(11,16)}-{event.endDateTime.slice(11,16)} · {event.category}</small></div><div className="consistency-event-actions"><button type="button" className="text-button" onClick={() => onEdit(event)}>Edytuj</button>{event.source === 'UNIVERSITY_XLSX' && event.seriesKey ? <button type="button" className="text-button" onClick={() => onStudySeriesCorrect(event)}>Edytuj serię</button> : null}</div></div>)}</div> : null}
