...
import { buildWorkScheduleDiff, candidatesFromEmployee, coworkerDrafts, detectCandidateCollisions, employeeForProfile, formatWorkMinutes, sortCoworkerOverlaps } from './work.service';
    ...
    <section className="view-shell work-view">
      ...
      {workTab === 'availability' ? <AvailabilityView onDataChanged={async () => { await Promise.all([refresh(), onDataChanged()]); }} onOpenSettings={() => setSettingsOpen(true)} /> : workTab === 'summary' ? <WorkSummaryView workEvents={confirmedWorkEvents} workImports={imports} availabilityPlans={availabilityPlans} onImportClick={requestPdfImport} onOpenComparison={() => { setComparisonDetailsRequest((value) => value + 1); setWorkTab('schedule'); }} onOpenAvailability={() => setWorkTab('availability')} initialMonth={latestActive?.periodStart.slice(0, 7)} /> : <>
        ...
        {analysis ? <section className="panel work-analysis-card"><div className="panel-heading"><div><p className="section-kicker">Podgląd przed zapisem</p><h2>{analysis.activeImport ? 'Aktualizacja grafiku' : 'Nowy grafik'} - {formatPeriod(analysis.result.periodStart)}</h2></div><button type="button" className="text-button" onClick={() => setAnalysis(null)}>Anuluj</button></div>
          ...
          <details className="work-analysis-details"><summary>Sprawdź szczegóły importu</summary><div className="work-analysis-details-meta"><span>PDF: <strong>{sourceSum !== undefined ? formatWorkMinutes(sourceSum) : 'brak sumy'}</strong></span><span>Kolizje: <strong>{analysis.collisions.filter((item) => item.kind === 'OVERLAP').length}</strong></span><span>Zespół: <strong>{analysis.coworkers.length ? `${new Set(analysis.coworkers.map((item) => item.normalizedName)).size} osób` : 'wyłączony'}</strong></span></div>
        ...
        <section className="panel work-next-card"><div className="panel-heading compact-heading"><div><span className="section-kicker">Najbliższa zmiana</span><h2>{nearestEvent ? `${formatDate(nearestEvent.startDateTime.slice(0,10))}, ${nearestEvent.startDateTime.slice(11,16)}-${nearestEvent.endDateTime.slice(11,16)}` : 'Brak nadchodzącej zmiany'}</h2></div></div>{nearestEvent ? <div className="next-work-shift"><span>{locations.find((location) => location.id === nearestEvent.locationId)?.name ?? profile?.workplaceName ?? 'Praca'}</span>{nearestCoworkers.length ? <div className="coworker-inline"><small>Z Tobą:</small>{nearestCoworkers.slice(0, 5).map((person) => <span key={`${person.displayName}-${person.coworkerStartTime}-${person.coworkerEndTime}`}>{person.displayName}</span>)}{nearestCoworkers.length > 5 ? <span className="coworker-more">+{nearestCoworkers.length - 5}</span> : null}</div> : <small className="muted-copy">Brak zapisanych nakładających się zmian zespołu.</small>}</div> : <p className="muted-copy">Po imporcie PDF najbliższa zmiana pojawi się tutaj.</p>}</section>

        <AvailabilityWorkComparisonPanel plans={availabilityPlans} workEvents={workEvents} workImports={imports} onImportClick={requestPdfImport} onOpenAvailability={() => setWorkTab('availability')} openDetailsRequest={comparisonDetailsRequest} />
