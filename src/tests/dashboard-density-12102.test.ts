...
describe('1.2.0.102 dashboard density', () => {
  it('keeps Study group choice visible without rendering the full option matrix', () => {
    expect(study).toContain('PROFILE_PARTITIONS');
    expect(study).toContain('study-group-dashboard');
    expect(study).toContain('study-group-dashboard-card');
    expect(study).toContain('<select value={current}');
    expect(study).toContain('setPartitionGroup');
  ...
  it('keeps readable mobile controls while collapsing dashboard columns', () => {
    expect(styles).toContain('.study-group-dashboard-card select { min-height: 42px; }');
