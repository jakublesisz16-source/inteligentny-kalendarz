...
check('Today next strip no longer repeats category', !today.includes('nextCategoryLabels') && today.includes('today-next-strip'));
...
check('Availability copy is reduced to natural labels', availability.includes('<h3>Automat</h3>') && availability.includes('>Wolne</span>') && availability.includes('<strong>Wyjątki</strong>') && !availability.includes('automat analizuje kalendarz'));
