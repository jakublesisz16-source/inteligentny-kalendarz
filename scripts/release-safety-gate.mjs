...
assert(securityReleaseGate.includes("relativePath: 'licencjat-ii-rok-piel.-25.09.2026.xls'") && securityReleaseGate.includes('sizeBytes: 148_992') && securityReleaseGate.includes('b6279b96e8cdfc7a95b9c7199686db424ef84e315215a03545957aee413964e4'), 'Build232 security gate must allow only the exact verified local Study source identity');
...
assert((workView.includes('<summary>{formatPersonCount(coworkers.length)}</summary>') || (workView.includes('work-shift-row-trigger') && workView.includes('work-shift-team-count'))) && coworkerList.includes('coworker-compact-time'), 'Build209 compact Work teammate expansion missing');
...
assert(financeDashboard.includes('>Transakcje</button>') && financeDashboard.includes('finance-items-review-badge') && financeDashboard.includes("setExpenseListMode('ITEMS')"), 'Finance transaction/item drill-down switch is missing');
...
assert(financeDashboard.includes('placeholder="Szukaj pozycji"') && financeDashboard.includes('finance-month-summary-compact') && financeDashboard.includes('finance-month-category-chips') && financeDashboard.includes('formatShortDate(receipt.date)'), 'monthly Finance copy or compact category/transaction layout regressed');
...
const verifiedStudyPlan = source('src/study/verified-study-plan.ts');
const verifiedStudyPlanTest = source('src/tests/verified-study-plan.test.ts');
...
assert(verifiedStudyPlan.includes('b6279b96e8cdfc7a95b9c7199686db424ef84e315215a03545957aee413964e4') && verifiedStudyPlan.includes('ba9be8db4573567662b6fa2eba588e486b32709f68535c2fe4fcdeb903df443e') && verifiedStudyPlan.includes('a2df44543641c2f699d8a053a981eb2a6cc25c9f5fa7584082b5ad6bbe0a66f6'), 'Build228 verified Study runtime reference drifted from the audited 25.09.2026 source');
...
if (currentState) assert(Array.isArray(currentState.activeStudyQaProfile?.selectedGroups) && currentState.activeStudyQaProfile.selectedGroups.length === 3 && currentState.activeStudyQaProfile.importableCount === 76 && currentState.activeStudyQaProfile.incompleteCount === 3, 'active Study QA profile drifted from the audited 25.09.2026 source');
if (currentState) assert(currentState.activeStudySource === 'licencjat-ii-rok-piel.-25.09.2026.xls' && currentState.activeStudySemanticReference?.allCandidatesSha256 === 'ba9be8db4573567662b6fa2eba588e486b32709f68535c2fe4fcdeb903df443e' && currentState.activeStudySemanticReference?.selectedProfileCandidatesSha256 === 'a2df44543641c2f699d8a053a981eb2a6cc25c9f5fa7584082b5ad6bbe0a66f6', 'active Study source semantic reference drifted');
