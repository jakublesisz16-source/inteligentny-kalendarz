...
const publicRepoDoc = source('docs/PUBLIC_REPOSITORY.md');
...
assert(version.includes("APP_VERSION = '1.2.0'"), 'APP_VERSION drifted');
...
assert(buildInfo.appVersion === '1.2.0' && buildInfo.channel === 'public' && buildInfo.packageVersion === '1.2.0' && buildInfo.date === '2026-09-13', 'BUILD_INFO.json is not synchronized with public 1.2.0 metadata');
assert(packageJson.version === '1.2.0', 'package.json version drifted');
assert(!projectRootEntries.some((name) => /^HANDOFF_NEW_CHAT_.*\.md$/u.test(name)), 'public root must not contain handoff files');
assert(!projectRootEntries.some((name) => /^PRIVATE_.*\.md$/u.test(name)), 'public root must not contain PRIVATE notes');
...
assert(!publicRepoDoc.includes('Finalna architektura `1.0.0`'), 'public repository guide still presents obsolete 1.0.0 wording as current');
...
assert(eventCard.includes('event-coworker-mobile-more'), 'mobile calendar coworker compaction helper missing');
assert(workView.includes('nearestCoworkers.slice(0, 5)') && workView.includes('coworker-more'), 'Work nearest-shift teammate summary is not compact');
...
assert(financeDashboard.includes("tripCurrency(activeTripDefinition) !== 'PLN'"), 'foreign-trip OCR safety guard is missing');
...
assert(privacyDoc.includes('## Finanse i waluty') && privacyDoc.includes('zapisanym lokalnie') && privacyDoc.includes('nie wykonuje requestów do zewnętrznego API'), 'local FX privacy boundary is not documented');
...
assert(sw.includes("const CACHE_NAME = `${CACHE_PREFIX}v1.2.0`"), 'current service worker cache version missing');
assert(audit.includes("const CACHE_NAME = `${CACHE_PREFIX}v1.2.0`"), 'production audit still expects an obsolete service worker revision');
